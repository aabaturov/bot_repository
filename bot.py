...
def repeat(message):
    ...
    if message.text == "Photo" and status == 0:
        ...
        if status == 0:
            bot.send_message(message.chat.id, 'В данном разделе меню Вы можете:\n1) Перейти в раздел отправки фото (прислать слово "Photo", либо нажать соответствующую кнопку в меню.\n2) Попросить рассказать про хобби (прислать слово "Hobby", либо нажать соответствующую кнопку в меню.\n3) Перейти в раздел отправки аудио (прислать слово "Voice", либо нажать соответствующую кнопку в меню.\n')
        if status == 1:
            bot.send_message(message.chat.id, 'В данном разделе меню Вы можете:\n1) Попросить отправить последнее селфи (прислать слово "Selfie", либо нажать соответствующую кнопку в меню.\n2) Попросить прислать фото из старшей школы (прислать слово "High school", либо нажать соответствующую кнопку в меню.\n3) Вернуться в главное меню (прислать слово "Back", либо нажать соответствующую кнопку в меню.')
        if status == 2: 
            bot.send_message(message.chat.id, 'В данном разделе меню Вы можете:\n1) Попросить объяснить бабушке, что такое ChatGPT (прислать команду "GPT explain", либо нажать соответствующую кнопку в меню.\n2) Попросить объяснить бабушке разницу SQL и NoSQL (прислать команду "SQL/NoSQL diff", либо нажать соответствующую кнопку в меню.\n3) Прислать любовную историю (прислать команду "Lovestory", либо нажать соответствующую кнопку в меню.\n4) Вернуться в главное меню (прислать слово "Back", либо нажать соответствующую кнопку в меню.')
